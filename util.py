...
def independence_of_errors_assumption(model, features, labels, plot=True):
    ...
    elif dw_value > 2.5: autocorrelation = 'negative'
...
def get_rsquared_adj1(model, features, labels):
...
def get_rsquared_adj_r(model, features, labels):
    '''Returns adjusted r^2 score.'''
    num_attributes = features.shape[1]
    y_pred = model.predict(features)  # Direktno predviđanje bez dodavanja konstante
...
def remove_outliers_iqr(df, columns=None, threshold=1.5):
    ...
    - columns (list): List of columns to consider for outlier removal. If None, consider all numeric columns.
    - threshold (float): Threshold for defining outliers. The default is 1.5.
